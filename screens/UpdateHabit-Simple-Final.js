...
export default function UpdateHabitSimpleFinal({ route, navigation }) {
  // Get habit from route params
  const { habit } = route?.params || {};
  
  // If no habit provided, go back
  if (!habit) {
  ...
  const handleUpdateHabit = async () => {
    ...
    try {
      // Update the habit
      await updateHabit(habit.id, habitName.trim(), selectedColor, description.trim());
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.previewCard, { borderLeftColor: selectedColor }]}>
          ...
          <View style={styles.streakInfo}>
            ...
            <Text style={styles.streakText}>{habit.streak || 0} day{(habit.streak || 0) !== 1 ? 's' : ''} streak</Text>
          </View>
        </View>
